...
y_shift = 0.1
camera = Camera(np.array([-3.2, -0.1 + y_shift, -0.5], dtype=float),
                np.array([-3.2, -0.1 + y_shift, 0.5]),
                np.array([-2.88, -1.04 + y_shift, 0.5]), 50)  # change angle of view
surfaces = []
...
sides_surfaces = [
    ...
    *get_triangle_equation(points[0], points[1], points[2]),
    *get_triangle_equation(points[4], points[3], points[5])
]
...
surfaces.append(SolidSurface(get_sphere_equations(np.array([3.05, -0.57 + y_shift, 0]), 0.3), np.array([1, 1, 1]), 1))
...
color_split = 1
render_scene(f"prism_{color_split}.png", surfaces, camera, 1000, 1000, color_split=color_split)
