...
def get_sphere_equations(center_at: np.array, radius: float) -> list[SurfaceEquation]:
    equations = []
    equations.append(SurfaceEquation((x-center_at[0])**2+(y-center_at[1])**2+(z-center_at[2])**2-radius*radius, []))
    return equations
...
def get_triangle_equation(point0: np.array, point1: np.array, point2: np.array) -> list[SurfaceEquation]:
    ...
    equations.append(SurfaceEquation((x - point0[0]) * normal[0] + (y - point0[1]) * normal[1] + (z - point0[2]) * normal[2], [
